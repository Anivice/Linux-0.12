...
dd if=$bootsect bs=512 count=1 of=$IMAGE 2> /dev/null > /dev/null

# Write setup(4 * 512bytes, four sectors) to stdout
[ ! -f "$setup" ] && echo "there is no setup binary file there" && exit -1
dd if=$setup seek=1 bs=512 count=4 of=$IMAGE > /dev/null 2> /dev/null
...
dd if=$system seek=5 bs=512 count=$((2888-1-4)) of=$IMAGE > /dev/null 2> /dev/null

# Set "device" for the root image file
echo -ne "\x$DEFAULT_MINOR_ROOT\x$DEFAULT_MAJOR_ROOT" | dd ibs=1 obs=1 count=2 seek=508 of=$IMAGE conv=notrunc > /dev/null 2> /dev/null
